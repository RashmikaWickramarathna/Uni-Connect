...
export default function AdminCalendar({ events }) {
  ...
  const dot = status => {
    ...
    if (normalizedStatus === 'approved') return '#16a34a';
    if (normalizedStatus === 'rejected') return '#dc2626';
    if (normalizedStatus === 'cancelled') return '#64748b';
    return '#d97706';
  };
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '20px', alignItems: 'start' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: '4px' }}>
          {cells.map((day, idx) => {
            ...
            return (
              ...
              >
                <span style={{ fontSize: '13px', fontWeight: isToday ? 800 : hasEvents ? 700 : 400, color: isToday ? '#2563eb' : hasEvents ? '#16a34a' : '#374151', display: 'block' }}>
                  {day}
                </span>

                {hasEvents && (
                  <div style={{ display: 'flex', justifyContent: 'center', gap: '2px', marginTop: '4px', flexWrap: 'wrap' }}>
                    {dayEvs.slice(0, 3).map((event, index) => (
                      <div key={index} style={{ width: '6px', height: '6px', borderRadius: '50%', background: dot(event.status) }} />
                    ))}
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '14px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', minHeight: '280px' }}>
        <h3 style={{ fontFamily: 'Syne, sans-serif', fontSize: '15px', fontWeight: 800, marginBottom: '4px', color: '#0f172a' }}>
          ...
          return (
            <div
              ...
              style={{
                ...
                marginBottom: '14px',
                paddingBottom: '14px',
                borderBottom: index < selEvs.length - 1 ? '1px solid #f1f5f9' : 'none',
              }}
            >
              ...
              <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '2px' }}>
                {fmtTime(event.time)} {fmtTime(event.time) ? '-' : ''} {safeText(event.venue).trim() || 'TBA'}
              </div>
              <div style={{ fontSize: '11px', color: '#94a3b8', marginBottom: '6px' }}>
                {safeText(event.organizer).trim() || 'Unknown Society'}
              </div>
              <span style={{ fontSize: '11px', fontWeight: 700, color: statusColor, background: `${statusColor}15`, padding: '2px 10px', borderRadius: '20px' }}>
