  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000, padding: "20px" }}>
      <div style={{ background: "#fff", borderRadius: "20px", padding: "32px", width: "100%", maxWidth: "480px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: "12px", padding: "14px 16px", marginBottom: "22px" }}>
          <p style={{ fontWeight: 700, color, fontSize: "15px", marginBottom: "4px" }}>{isDelete ? "Delete Event" : "Reject Event"}</p>
          <p style={{ fontSize: "13px", color: "#64748b" }}>
            You are about to {isDelete ? "delete" : "reject"} <strong>"{eventTitle}"</strong>.
            The society will be notified with your reason.
          </p>
        </div>
        ...
        <textarea
          ...
          placeholder={isDelete ? "e.g. Event violates university policy..." : "e.g. Venue not available, please choose another date..."}
          style={{
            width: "100%",
            minHeight: "110px",
            padding: "12px 14px",
        ...
        <div style={{ display: "flex", gap: "12px", marginTop: "22px" }}>
          <button onClick={confirm} style={{ flex: 1, padding: "11px", background: color, border: "none", borderRadius: "10px", color: "#fff", fontWeight: 700, cursor: "pointer", fontSize: "14px" }}>
            Confirm {isDelete ? "Delete" : "Reject"}
          </button>
          <button onClick={onCancel} style={{ flex: 1, padding: "11px", background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "10px", color: "#374151", fontWeight: 600, cursor: "pointer", fontSize: "14px" }}>
            Cancel
          </button>
        </div>
      </div>
