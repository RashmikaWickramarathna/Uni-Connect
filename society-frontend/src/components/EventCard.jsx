    ...
    <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:'12px',marginBottom:'16px',overflow:'hidden',boxShadow:'0 1px 4px rgba(0,0,0,0.04)',borderLeft:`4px solid ${cc}`,transition:'box-shadow 0.2s'}}
      ...
      {img && (
        <div style={{position:'relative'}}>
          <img src={img} alt={event.title} style={{width:'100%',height:'200px',objectFit:'cover',display:'block'}} />
      ...
      <div style={{padding:'18px 20px'}}>
        <div style={{display:'flex',justifyContent:'space-between',gap:'16px',flexWrap:'wrap'}}>
          <div style={{flex:1}}>
            ...
            <h3 style={{fontSize:'16px',fontWeight:700,color:'#0f172a',marginBottom:'6px',fontFamily:'Syne, sans-serif'}}>{event.title}</h3>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'8px',minWidth:'130px'}}>
            <button onClick={()=>onEdit(event)} disabled={event.status==='approved'} style={{padding:'8px 12px',background:event.status==='approved'?'#f1f5f9':'#f8fafc',border:'1px solid #e2e8f0',borderRadius:'7px',color:event.status==='approved'?'#cbd5e1':'#374151',cursor:event.status==='approved'?'not-allowed':'pointer',fontSize:'13px',fontWeight:600,fontFamily:'Space Grotesk, sans-serif',textAlign:'center'}}>
              Edit
            </button>
            <button onClick={()=>onDelete(event._id)} style={{padding:'8px 12px',background:'#fef2f2',border:'1px solid #fca5a5',borderRadius:'7px',color:'#dc2626',cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:'Space Grotesk, sans-serif',textAlign:'center'}}>Delete</button>
            <button onClick={()=>onGeneratePost(event)} style={{padding:'8px 12px',background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:'7px',color:'#2563eb',cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:'Space Grotesk, sans-serif',textAlign:'center'}}>Generate Post</button>
          </div>
        </div>
      </div>
    </div>
