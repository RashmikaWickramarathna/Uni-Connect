    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:'20px',alignItems:'start'}}>
      <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:'14px',padding:'24px',boxShadow:'0 1px 4px rgba(0,0,0,0.04)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
          <h2 style={{fontFamily:'Syne, sans-serif',fontSize:'18px',fontWeight:800,color:'#0f172a'}}>My Event Calendar</h2>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'4px'}}>
          {cells.map((day,idx)=>{
            ...
            const isToday=key===todayStr, hasEv=dayEvs.length>0, isSel=sel===key;
            return (
        ...
        {selEvs.map((ev,i)=>{
          ...
          return (
            <div key={i} style={{borderLeft:`3px solid ${sc}`,paddingLeft:'12px',marginBottom:'14px',paddingBottom:'14px',borderBottom:i<selEvs.length-1?'1px solid #f1f5f9':'none'}}>
              ...
              {ev.status==='rejected'&&ev.adminReason&&<p style={{fontSize:'11px',color:'#dc2626',marginTop:'4px'}}>Reason: {ev.adminReason}</p>}
            </div>
          );
        })}
      </div>
