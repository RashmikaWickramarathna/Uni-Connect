...
const catColors = { Academic:'#2563eb', Sports:'#16a34a', Cultural:'#d97706', Social:'#db2777', Workshop:'#7c3aed', Other:'#64748b' };
const statusCfg = {
...
export default function AdminEventCard({ event, onApprove, onReject, onDelete, onEdit }) {
  const st = statusCfg[event.status] || statusCfg.pending;
  const cc = catColors[event.category] || '#64748b';
  const img = getImageUrl(event.image);

  const fmtDate = d => { if (!d) return ''; const [y,m,day] = d.split('-'); return new Date(y,m-1,day).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'}); };
        ...
        <div style={{ display:'flex', justifyContent:'space-between', gap:'16px', flexWrap:'wrap' }}>
          <div style={{ flex:1 }}>
            <div style={{ display:'flex', gap:'8px', marginBottom:'8px', flexWrap:'wrap', alignItems:'center' }}>
              ...
              {days !== null && days > 0 && days <= 7 && (
                <span style={{ background:'#fef3c7', color:'#d97706', border:'1px solid #fcd34d', padding:'3px 10px', borderRadius:'20px', fontSize:'11px', fontWeight:700 }}>
                  {days === 1 ? 'Tomorrow' : `In ${days} days`}
                </span>
              )}
              {days !== null && days === 0 && (
                <span style={{ background:'#fef2f2', color:'#dc2626', border:'1px solid #fca5a5', padding:'3px 10px', borderRadius:'20px', fontSize:'11px', fontWeight:700 }}>Today</span>
              )}
            </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'8px', minWidth:'110px' }}>
            {event.status !== 'approved' && (
              <button onClick={() => onApprove(event._id)} style={btn('#f0fdf4','#16a34a','#86efac')}>Approve</button>
            )}
            {event.status !== 'rejected' && (
              <button onClick={() => onReject(event)} style={btn('#fffbeb','#d97706','#fcd34d')}>Reject</button>
            )}
...
const btn = (bg,color,border) => ({ padding:'8px 12px', background:bg, border:`1px solid ${border}`, borderRadius:'7px', color, cursor:'pointer', fontSize:'13px', fontWeight:600, fontFamily:'Space Grotesk, sans-serif', textAlign:'center', transition:'opacity 0.15s' });