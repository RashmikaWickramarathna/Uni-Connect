...
export default function Analytics() {
  ...
  const rejectionRate = data.total > 0 ? Math.round((data.rejected / data.total) * 100) : 0;
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'16px', marginBottom:'24px' }}>
        {[
          { label:'Total Events', value:data.total, color:'#2563eb' },
          { label:'Approved', value:data.approved, color:'#16a34a', sub:`${approvalRate}% approval rate` },
          { label:'Rejected', value:data.rejected, color:'#dc2626', sub:`${rejectionRate}% rejection rate` },
          { label:'Upcoming (30 days)', value:data.upcoming, color:'#7c3aed' },
        ].map(s => (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginBottom:'24px' }}>
            ...
            {monthEntries.map(([key, count]) => {
              ...
              return (
                <div key={key} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:'6px', height:'100%', justifyContent:'flex-end' }}>
                  ...
                  <div style={{ width:'100%', background:'#2563eb', borderRadius:'4px 4px 0 0', height:`${heightPct}%`, minHeight: count > 0 ? '4px' : '0', transition:'height 0.3s' }} />
                  <span style={{ fontSize:'10px', color:'#94a3b8', fontWeight:600 }}>{monthNames[month]}</span>
                </div>
              );
            })}
          </div>
        ...
        {/* Events by Category horizontal bars */}
        <div style={{ background:'#fff', border:'1px solid #e2e8f0', borderRadius:'12px', padding:'22px', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
            ...
            return (
              <div key={cat} style={{ marginBottom:'14px' }}>
                ...
                <div style={{ background:'#f1f5f9', borderRadius:'4px', height:'8px', overflow:'hidden' }}>
                  <div style={{ width:`${widthPct}%`, background:color, height:'100%', borderRadius:'4px', transition:'width 0.4s' }} />
                </div>
              </div>
            );
          })}
      ...
      {/* Status breakdown */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginBottom:'24px' }}>
        {/* Approval rate donut-style display */}
        <div style={{ background:'#fff', border:'1px solid #e2e8f0', borderRadius:'12px', padding:'22px', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
          ...
          {data.topSocieties.map((s, i) => (
            <div key={s.email} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 0', borderBottom: i < data.topSocieties.length-1 ? '1px solid #f1f5f9' : 'none' }}>
              ...
              <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
                ...
                <span style={{ fontSize:'18px', fontWeight:800, color:'#2563eb', fontFamily:'Syne, sans-serif' }}>{s.count}</span>
              </div>
            </div>
          ))}
        </div>
