...
export default function AdminCalendar({ events }) {
  ...
  const dotColor = s => s==='approved'?'#16a34a':s==='rejected'?'#dc2626':'#d97706';
    ...
    <div style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:'20px', alignItems:'start' }}>
      <div style={{ background:'#fff', border:'1px solid #e2e8f0', borderRadius:'14px', padding:'24px', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
          <h2 style={{ fontFamily:'Syne, sans-serif', fontSize:'18px', fontWeight:800 }}>Event Calendar</h2>
          <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
            <button onClick={() => setCur(c => { const d=new Date(c.year,c.month-1); return {month:d.getMonth(),year:d.getFullYear()}; })} style={navBtn}>&#8249;</button>
            <span style={{ fontWeight:700, fontSize:'14px', minWidth:'130px', textAlign:'center' }}>{MONTHS[month]} {year}</span>
            <button onClick={() => setCur(c => { const d=new Date(c.year,c.month+1); return {month:d.getMonth(),year:d.getFullYear()}; })} style={navBtn}>&#8250;</button>
          </div>
        </div>
          ...
          {cells.map((day,idx) => {
            ...
            return (
              <div key={key} onClick={() => setSel(isSel?null:key)}
                style={{ minHeight:'50px', padding:'6px 4px', borderRadius:'8px', textAlign:'center', cursor:hasEv?'pointer':'default', background:isSel?'#eff6ff':hasEv?'#f0fdf4':isToday?'#f8fafc':'transparent', border:isSel?'2px solid #2563eb':isToday?'2px solid #2563eb':hasEv?'1px solid #86efac':'1px solid transparent', transition:'all 0.15s' }}>
                ...
                {hasEv && <div style={{ display:'flex', justifyContent:'center', gap:'2px', marginTop:'4px', flexWrap:'wrap' }}>
                  {dayEvs.slice(0,3).map((ev,i) => <div key={i} style={{ width:'6px', height:'6px', borderRadius:'50%', background:dotColor(ev.status) }} />)}
      ...
      <div style={{ background:'#fff', border:'1px solid #e2e8f0', borderRadius:'14px', padding:'20px', boxShadow:'0 1px 4px rgba(0,0,0,0.04)', minHeight:'280px' }}>
        <h3 style={{ fontFamily:'Syne, sans-serif', fontSize:'15px', fontWeight:800, marginBottom:'4px' }}>{sel ? `Events on ${sel}` : 'Select a Date'}</h3>
        ...
        {selEvs.map((ev,i) => {
          ...
          return (
            <div key={i} style={{ borderLeft:`3px solid ${sc}`, paddingLeft:'12px', marginBottom:'14px', paddingBottom:'14px', borderBottom:i<selEvs.length-1?'1px solid #f1f5f9':'none' }}>
              ...
              <div style={{ fontSize:'12px', color:'#64748b', marginBottom:'2px' }}>{ev.time} — {ev.venue}</div>
              <div style={{ fontSize:'12px', color:'#94a3b8', marginBottom:'6px' }}>{ev.organizer}</div>
...
const navBtn = { background:'#f8fafc', border:'1px solid #e2e8f0', borderRadius:'6px', color:'#374151', cursor:'pointer', width:'30px', height:'30px', fontSize:'16px', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'Space Grotesk, sans-serif' };